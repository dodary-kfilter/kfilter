...
const RULES=[
 ...
 ['자료대상',   t=>{ if(!t.includes('시장 뉴스 제목')) return false; if(t.includes('후보 전체의 개요')) return t.includes('상세 자료에는 사업 요약') && t.includes('부문별 매출과 수주') && t.includes('최근 공시와 뉴스 제목') && t.includes('상세 자료는 종목당 4KB 안팎');
 ...
 ['배분',       t=>t.includes('본질은 자료의 사업 요약에서, 배경은 시장·금리 값에서 나오고, 기타는 한 줄로 끝납니다. 시간과 분량은 특징과 전망에 씁니다.')
 ...
 ['옛문구없음', t=>!/얻을 것이 잃을 것보다|값을 부를|이 종목에 맞는 구성으로|절대PER과 동종|구분이 필요합니다|직접 확인이 필요합니다|재무제표 밖에 있는|자료에 없는 값은|한 차례|이 목록 안에서의 비교|같은 방식으로 판단|배수 하나로|전체를 놓고|따라오는 모습|시장과 같은 견해라면|명령은 한 번만|가장 먼저 아래 명령|\[대상 \d+종목\]|\[스캔 지표|report-data\/|계측|수급 파일에 이미 있다|— 두세 줄|웹 검색으로 확인하십시오|종합적 판단으로 기대수익을 정하고|알기 어렵습니다|이 자리에서 모두 제시|과거, 현재, 그리고 앞으로|그것의 현재 상태|지금 판단을 가르는 요인|과거와 현재|향후 전망 —/.test(t)],
];

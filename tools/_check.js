...
const RULES=[
 ...
 ['실행제약1',     t=>t.includes('[쓰기 전에]') ? (t.match(/명령은 한 번이고/g)||[]).length===1 : t.includes('[운용]') ? (t.match(/\[운용\]/g)||[]).length===1 : (t.match(/\[★출력 도중 도구 호출 금지\]/g)||[]).length===1],
 ['점검줄1',      t=>t.includes('[쓰기 전에]') ? true : (t.match(/\[점검\]/g)||[]).length===1],
